...
test.describe('albums', () => {
  ...
  test('creates an album and lists it', async ({ page }) => {
    ...
    // The column keeps its own default. Writing one from the browser would
    // record a choice the interface no longer offers.
    expect(insert?.body).not.toHaveProperty('layout')
  })

  test('offers no layout to choose when creating an album', async ({ page }) => {
    await signIn(page)

    await page.getByRole('button', { name: 'Start your first album' }).click()

    await expect(page.getByRole('radio', { name: 'Masonry' })).toHaveCount(0)
    await expect(page.getByRole('radio', { name: 'Grid' })).toHaveCount(0)
  })
  ...
  test('opens an album and returns to the library', async ({ page }) => {
  ...
  test('an open album offers no layout switch, and writes no layout', async ({ page }) => {
    ...
    // while Move earlier / Move later promise it runs across the rows. The
    // column and its check constraint stay in the database; nothing writes them.
    const calls = await signIn(page, { albums: [albumRecord()] })

    await page.getByRole('button', { name: /Summer by the lake/ }).click()
    await expect(page.getByRole('heading', { name: 'No photos yet' })).toBeVisible()

    await expect(page.getByRole('group', { name: 'Album layout' })).toHaveCount(0)
    await expect(page.getByRole('button', { name: 'Masonry' })).toHaveCount(0)
    await expect(page.getByRole('button', { name: 'Grid' })).toHaveCount(0)

    expect(calls.all.filter((call) => call.method === 'PATCH')).toHaveLength(0)
  })
